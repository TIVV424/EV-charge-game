...
def best_response_station_joint(
    ...
    def neg_profit(x):
        ...
        flows = solve_sue_msa(demand, stations, theta, tau, alpha, tax_list)
        # add limit for sum capacity

        q_j = flows[j]
        wait_time = max(0, q_j - c_j) / c_j
        wait_time_penalty = tau * wait_time
        fixed_cost = fixed_cost_rate[j-1] * c_j
...
def nash_equilibrium(
    ...
    for it in range(max_iter):
        ...
        for j in stations.keys():
            ...
            max_capacity_ub_j = max_capacity - sum(stations["capacity"] for k, stations in stations.items() if k != j)
